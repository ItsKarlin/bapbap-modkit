...
- ✅ **Install 404, the other half** — reported from a friend's Windows install: every download from *our* catalog 404'd. Same root cause as the BAPHub 404 above, mirrored — the manager resolves `sourcePath` against the version manifest's folder, but `tools/add-mod.sh` wrote `dist/<file>.dll` and `docs/catalog-schema.md` claimed it resolved against `baseUrl`, so the client asked for `catalog/<id>/<ver>/dist/<file>.dll`. Nobody caught it because ACT 5 was only ever verified end-to-end against BAPHub packages, never against one of our own. Fixed in data, so no client update: each version's DLL now ships beside its own `version.json` and `sourcePath` is a bare filename — which also freezes the published artifact, since `dist/` is overwritten by the next build. `add-mod.sh` copies it there and now validates that the exact file the manager will fetch exists. DONE (2026-07-31 20:46:42)
- ⬜ **Long error text does not wrap in browse rows** — known since 2026-07-28, now seen in the wild: the friend's 404 message ran off both screen edges, so the part that named the missing file was unreadable. The error a user can't read is the error you get told about second-hand. Wrap it, or truncate with the URL tail kept.
...
- 🔄 **Round Mutators** — unlock the devs' 16 built-in `GM_*` modifiers in normal lobbies. IDs 0–15 captured. **API found in the dump 2026-07-28 (`GameMode.cs`), and it is all public and server-authoritative:** `enableGameModifiers` (gate), `SvAddGameModifier(int)`, `SvRemoveGameModifier(int)`, `InitializeGameModifiersMatchMaking(int[])`, with `RpcAddGameModifier` replicating to clients. No Harmony patch, no reimplemented effects, no touching `devLobbyGameModifierPool` — the host sets them and the game syncs everyone, so guests still install nothing.
  **Spec agreed:** host scope, assembly `BAPBAPRoundMutators`, id `itskarlin.bapbap.round-mutators`. Everything configurable — `SelectionMode` (PerRound/Match/Escalating), `MaxActive`, `Announce`, plus one pool toggle per modifier generated from the ID list rather than typed out.
  ✅ **Built and published as v0.1.0** (2026-07-28 10:30) — `src/RoundMutators/`, in the catalog, host-scope, four spectacle mutators enabled by default, every one a switch in its ini. Loads cleanly and writes its config.
  ⚠️ **NOT yet confirmed to actually apply a modifier in a live match.** That is the one open question; it logs each attempt by name so a refusal is visible.
  ⬜ **No round-start signal found** on `GameMode`, so mutators are chosen once per match rather than per round. `InitializeRoundAugmentSelection` is the most promising lead.
...
## Changelog
- **2026-08-01 04:33:34** — Handoff rewritten around the 404. Two things the friend's screenshot also proved, logged rather than fixed: the manager itself **runs on Windows** (they reached the BROWSE tab and got a real error back from a real fetch — how they installed it is still unasked), and **long errors do not wrap**, which is why the message ran off both edges. `add-mod.sh` was the only publish path and it was wrong, so this was never a one-package bug.
- **2026-07-31 20:46:42** — First outside user, first bug: a friend on Windows got a 404 downloading Third Person. Every package in *our* catalog was unreachable — `sourcePath` was published as `dist/<file>.dll` while the manager resolves it against the version manifest's folder, and the schema doc documented the wrong one. Fixed purely in catalog data, so nobody needs a new manager: the DLLs now sit beside their `version.json`, `add-mod.sh` puts them there and validates the fetch path, and the doc was corrected. Verified live against raw.githubusercontent — both packages 200 with hashes matching their manifests. Lesson: ACT 5 was verified end-to-end against BAPHub's packages but never once against our own.
- **2026-07-29 03:46:00** — Session close. Manager **v0.3.0**: scope now comes from install receipts rather than a hardcoded list of mod names, removing the last per-mod hardcoding; word-value settings became tap-to-cycle pickers and keybinds became rebindable, unsticking 14 settings across 5 installed mods that could not be changed in-game at all. Round Mutators v0.1.0 built and published to the catalog. The settings panel was fixed a fourth time by **deleting the fade entirely** — it caused three of the four breakages and was never needed, since our panel is opaque and drawn last. `tools/add-mod.sh` publishes a mod in one command.
