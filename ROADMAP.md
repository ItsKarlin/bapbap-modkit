...
- ⬜ **Fetch layer** — download `packages.json` / `version.json` / the DLLs from inside the game. `UnityWebRequest` vs `HttpClient` under IL2CPP is untested; must not block the main thread.
- ⬜ **Install/uninstall execution** — stage all files, verify every sha256, then move; keep the ini on uninstall.
- ⬜ **Downloader UI** — browse list with Installed / Install / Update, requirement warnings rendered from `severity`.
